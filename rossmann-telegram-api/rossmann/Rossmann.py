...
class Rossmann( object ):
    def __init__( self ):
        #self.home_path='C:/repos/Rossmann_Project/'
        self.home_path = ''
    ...
    # Dicinário de parâmetros Self (variáveis da própria classe) para o pré-processamento dos dados, incluindo escalonamento e codificação de variáveis categóricas.
    def feature_engineering( self, df2 ):
        ...
        df2['day'] = df2['date'].dt.day
         ...
         # week of year
        df2['week_year'] = (df2['date'].dt.isocalendar().week.astype(int))
        ...
        df2['assortment'] = df2['assortment'].apply( lambda x: 
                                                    'basic' if x =='a' else 'extra' if x == 'b' else 'extended' )

        # state holiday
        df2['state_holiday'] = df2['state_holiday'].apply( lambda x:
                                                          'public_holiday' if x == 'a' else 'easter_holiday' if x == 'b' else 'christmas' if x == 'c' else 'regular_day' )
